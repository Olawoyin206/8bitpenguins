...
const SESSION_SIGN_PREFIX = '8bitPenguins Admin Session'
...
function parseSignedMessage(message) {
  ...
  if (lines[0] !== SESSION_SIGN_PREFIX) return null
  const parsed = {}
...
async function reserveNonce(address, nonce, issuedAt) {
  ...
  const key = `session:${String(address || '').toLowerCase()}:${normalizedNonce}`
  ...
  if (String(payload?.result || '') !== 'OK') {
    return { ok: false, reason: 'Session signature already used' }
  }
  return { ok: true }
}
...
export default async function handler(req, res) {
  ...
  if (!SESSION_SECRET) {
    res.status(500).json({ ok: false, error: 'Admin session secret is not configured' })
    return
  }
  ...
  try {
    ...
    if (!address || !signature || !message || !Number.isFinite(issuedAt) || issuedAt <= 0 || !nonce) {
      res.status(400).json({ ok: false, error: 'Missing admin session signature payload' })
      return
    }

    if (Math.abs(Date.now() - issuedAt) > SIGN_MAX_AGE_MS) {
      res.status(401).json({ ok: false, error: 'Admin session signature expired' })
      return
    }

    const parsedMessage = parseSignedMessage(message)
    if (!parsedMessage) {
      res.status(401).json({ ok: false, error: 'Invalid admin session signature message' })
      return
    }
    ...
    if (
      ...
      res.status(401).json({ ok: false, error: 'Admin session signature payload mismatch' })
      return
    }
    ...
    if (!recovered || recovered.toLowerCase() !== address.toLowerCase()) {
      res.status(401).json({ ok: false, error: 'Invalid admin session signature' })
      return
    }
    ...
    const nonceReservation = await reserveNonce(address, nonce, issuedAt)
    ...
    const token = jwt.sign(
      {
        ...
        host: expectedHost || signedHost,
        chainId: expectedChainId || signedChainId,
        contract: (expectedContract || signedContract).toLowerCase(),
      },
      SESSION_SECRET,
    ...
    res.status(200).json({
      ok: true,
      token,
      expiresInSec: SESSION_TOKEN_TTL_SECONDS,
    })
  } catch (error) {
    res.status(500).json({
      ok: false,
